...
+ (NSDate *)date:(NSString *)dateString dateFormatter:(NSString *)formate {
...
+ (NSDate*)getUTCDateFromServerDate : (NSString*) strDate
{
    
 //    CreatedOn = "2017-05-08T11:14:16.507Z";
    NSDateFormatter *dateFormate = [[NSDateFormatter alloc]init];
    //[dateFormate setTimeZone:[NSTimeZone timeZoneForSecondsFromGMT:0]];
    [dateFormate setDateFormat:@"yyyy-MM-dd'T'HH:mm:ss.SSSZ"];
    NSDate *UTCDate = [dateFormate dateFromString:strDate];
    
    [dateFormate setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
   // [dateFormate setTimeZone:[NSTimeZone systemTimeZone]];
    NSString *localDate = [dateFormate stringFromDate:UTCDate];
    NSLog(@"%@",localDate);
    return [dateFormate dateFromString:localDate];
...
+ (NSString*)getUTCDateFromServerDateNew : (NSString*) strDate {
    
    
    
 //   DOB = "1983-04-26T00:00:00";

    NSDateFormatter *dateFormate = [[NSDateFormatter alloc]init];
    //[dateFormate setTimeZone:[NSTimeZone timeZoneForSecondsFromGMT:0]];
    [dateFormate setDateFormat:@"yyyy-MM-dd'T'HH:mm:ss"];
    NSDate *UTCDate = [dateFormate dateFromString:strDate];
...
//self.lastActivityTimeStr = lastActivityTime;

//[dateFormate setTimeZone:[NSTimeZone timeZoneForSecondsFromGMT:0]]; //Global DATE TimeZone
//[dateFormate setDateFormat:[User dateFormate]];
//NSDate *UTCDate = [dateFormate dateFromString:self.lastActivityTimeStr];

+ (NSString *)dateFormateSNAP:(NSDate *)localDate {
    
     NSDateFormatter *dateFormatter = [NSDateFormatter new];
    [dateFormatter setDateFormat:@"MM/dd/yy"];
    NSString *local = [dateFormatter stringFromDate:localDate];
    return local ;
}


+ (NSString *)dateFormateMani:(NSDate *)localDate {
    
    NSDateFormatter *dateFormatter = [NSDateFormatter new]; //HH:mm:ss:SSS
    NSString *dateStr = @"";
    NSString *timeStr = @"";
    ...
    if ([localDate isToday]) {
        ...
        timeStr =  [dateFormatter stringFromDate:localDate];
    ...
    }else if ([localDate isThisWeek]) {
        [dateFormatter setDateFormat:@"EEE hh:mm a"];
        return [dateFormatter stringFromDate:localDate];
    }else if ([localDate isThisYear]) {
...
- (NSString *)displayFormateForDay {
    ...
    if ([date isToday]) {
        ...
        return [dateFormatter stringFromDate:date];
        //return [NSString stringWithFormat:@"%li %@ ago",(long)days,((days == 1)?@"day":@"days")];
