...
typealias FlashHandler = (_ camera : AVCaptureDeviceInput?) -> Void
...
class CameraMan {
  ...
  func switchCamera(handler:@escaping FlashHandler )
  {
    guard let currentInput = currentInput
      else {
        handler(nil)
  ...
  func takePhoto(_ previewLayer: AVCaptureVideoPreviewLayer, location: CLLocation?, completion: (() -> Void)? = nil) {
    ...
    func takePhoto(_ previewLayer: AVCaptureVideoPreviewLayer ,handler:@escaping ImageHandler ) {
        if take == true{
         take = false
